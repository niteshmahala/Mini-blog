...
const createAuthor = async function (req, res) {
  try {
    ...
    if(!isvalid(fname)){
      return res.send({status : false , msg :"firstname is required"})
    }


    if(!isvalid(lname)){
      return res.send({status : false , msg :"lastname is required"})
    }
...
try{
  ...
  if(!isvalid(email)){
    return res.send({status : false , msg :"emailt is required"})
  }
